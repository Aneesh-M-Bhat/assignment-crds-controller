...
func (c *Controller) syncHandler(ctx context.Context, key string) error {
	...
	deploymentName := foo.ObjectMeta.Name
	if deploymentName == "" {
	...
	if foo.Spec.Count != nil && *foo.Spec.Count != *deployment.Spec.Replicas {
		logger.V(4).Info("Update deployment resource", "currentReplicas", *foo.Spec.Count, "desiredReplicas", *deployment.Spec.Replicas)
		_, err = c.kubeclientset.AppsV1().Deployments(foo.Namespace).Update(context.TODO(), newDeployment(foo), metav1.UpdateOptions{})
	}
